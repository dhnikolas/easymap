...
const ifConditionPointerTemplate = `
	{{$nameIn:=.NameIn}}
	{{$structName:=.NameIn}}
	...
	if {{$parentInName}}{{$nameIn}} != nil {
		...
		{{range $field := .ListScalarFields }}
			{{$parentOutName}}{{$structName}}.{{$field.Name}} = {{$parentInName}}{{$nameIn}}.{{$field.Name}} 
		{{ end }}    
		{{ .Content  }} 	
	}
`
const slicePointerTemplate = `
	{{$nameIn:=.NameIn}}
	{{$structName:=.NameIn}}
	{{$parentInName:=.ParentInName}}
	{{$parentOutName:=.ParentOutName}}
	...
	for _, {{$nameIn}}Item := range {{$parentInName}}{{$nameIn}} {
		...
		{{range $field := .ListScalarFields }}
			new{{$nameIn}}.{{$field.Name}} = {{$nameIn}}Item.{{$field.Name}} 
		{{ end }}
		{{ .Content  }}
